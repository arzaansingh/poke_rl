"""
V4 Hyperparameter Grid Search Orchestrator
============================================
Runs all 4 models across every HP combination × multiple runs.
Each run uses a different randomly-sampled 9-pokemon pool, but
all 4 models in a run share the same pool.
...
from shared.config import (
    ...
    SHOWDOWN_DIR, EPS_DECAY_BATTLES, RUNS_PER_COMBO,
    build_grid, get_epsilon, get_pool_for_run,
)
...
def _draw_table_frame(combos):
    """Draw the static table frame once. Called at dashboard init."""
    global _TOTAL_LINES
    sys.stdout.write("\033[2J\033[H")  # Clear screen and reset cursor
    print(f"{'═' * _W}")
    print(f"  V4 GRID SEARCH   │   Initializing...".ljust(_W)) # Line 2
    print(f"{'═' * _W}")
    print(f"  {'Combo':<8} {'Alpha':>6} {'Gamma':>6} {'Lam':>5} │ {'FlatZ':>7} {'FlatS':>7} {'HierZ':>7} {'HierS':>7} │ {'Avg':>6} {'Runs':>8} {'Battles':>18} {'Status':<15}")
    print(f"  {'─' * (_W - 4)}")
    for _ in range(len(combos)):
        print() # Empty rows for data (Starts at Line 6)
    _TOTAL_LINES = 6 + len(combos)
    print(f"  {'─' * (_W - 4)}")
    print() # Summary Line
    print(f"{'═' * _W}")
...
def _draw_grid_dashboard(combos, start_time, initial_battles=0):
    ...
    # Update Header
    header = f"  V4 GRID SEARCH   │   {total_combos} combos × {RUNS_PER_COMBO} runs × 4 models × {BATTLES_PER_RUN:,} battles   │   Elapsed: {elapsed}"
    _write_line(2, header)
    ...
    for i, combo in enumerate(combos):
        ...
        battles_str = f"{combo_battles:,}/{total_for_combo:,}"

        line = (f"  {combo_id:<8} {combo['alpha']:>6.2f} {combo['gamma']:>6.3f} {combo['lam']:>5.1f}"
                f" │ {wr_strs[0]:>7} {wr_strs[1]:>7} {wr_strs[2]:>7} {wr_strs[3]:>7}"
                f" │ {avg_str:>6} {runs_done:>3}/{RUNS_PER_COMBO:<3} {battles_str:>18} {bar} {status:<5}")
        _write_line(_DATA_START + i, line)
    ...
    _write_line(_TOTAL_LINES + 1, summary_line)

    # Park cursor safely below the table to prevent text overlapping
    _move_to(_TOTAL_LINES + 4)
    sys.stdout.flush()
...
def run_single_combo(combo_id, combos, run_filter=None):
    ...
    # Start 4 servers (one per concurrent model)
    servers = []
...
def run_full_grid(max_parallel=8):
    ...
    print(f"V4 Grid Search: {len(combos)} combos × {RUNS_PER_COMBO} runs × {len(MODEL_NAMES)} models = {total_experiments} experiments")
    ...
    print(f"\nGRID SEARCH COMPLETE — {total_elapsed/60:.1f} minutes total")
    _print_summary(combos)
...
def _print_summary(combos=None):
    ...
    print(f"{'═' * 120}")
    print(f"  {'Rank':<5} {'Combo':<8} {'Alpha':>6} {'Gamma':>6} {'Lam':>5} {'Runs':>5}"
          f" │ {'FlatZ':>7} {'FlatS':>7} {'HierZ':>7} {'HierS':>7} │ {'AvgBest':>8}")
    print(f"  {'─' * 116}")
    ...
    for rank, row in enumerate(results[:15], 1):
        ...
        print(f"  {rank:<5} {row['combo_id']:<8} {row['alpha']:>6.2f} {row['gamma']:>6.3f} {row['lam']:>5.1f} {row['runs_done']:>3}/{RUNS_PER_COMBO}"
              f" │ {model_strs[0]:>7} {model_strs[1]:>7} {model_strs[2]:>7} {model_strs[3]:>7}"
              f" │ {row['avg_best']:>7.1%}")
    ...
    for model_name in MODEL_NAMES:
        ...
        if best_val:
            print(f"    {MODEL_LABELS[model_name]:<20} → {best_row['combo_id']} "
...
def _reset_grid():
    if not os.path.isdir(GRID_RESULTS_DIR):
    ...
    print(f"This will DELETE all grid results ({count} combo directories).")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="V4 Hyperparameter Grid Search")
    ...
    if args.list_grid:
        print(f"Grid: {len(combos)} combos × {RUNS_PER_COMBO} runs × 4 models = {len(combos) * RUNS_PER_COMBO * 4} experiments")
    ...
    if args.combo:
        ...
        run_full_grid(max_parallel=args.max_parallel)
