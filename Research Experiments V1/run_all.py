...
def _update_dashboard(processes, start_time):
    ...
    # Runs summary line (line 11) — shows per-run progress when in parallel-runs mode
    if _PARALLEL_RUNS_MODE:
        run_parts = []
        for model_name in MODEL_NAMES:
            short = MODEL_LABELS[model_name].split("+")[0].strip()[:4]  # "Flat" or "Hier"
            all_runs = _get_all_run_statuses(model_name)
            run_indicators = []
            for r_idx in range(RUNS_PER_MODEL):
                # Find status for this run
                r_status = None
                for rs in all_runs:
                    if rs.get('run_id') == r_idx + 1:
                        r_status = rs
                        break
                if r_status:
                    r_battles = r_status.get('battles', 0)
                    r_pct = min(r_battles / BATTLES_PER_RUN * 100, 100.0)
                    if r_pct >= 99.9:
                        run_indicators.append("✓")
                    else:
                        run_indicators.append(f"{r_pct:.0f}%")
                else:
                    # Check if CSV exists (completed run that cleaned up status)
                    log_file = os.path.join(EXPERIMENT_DIR, model_name, "logs", f"run_{r_idx + 1}.csv")
                    if os.path.exists(log_file):
                        ...
                        run_indicators.append("—")
            run_parts.append(f"{short}: [{' '.join(f'{r:>4}' for r in run_indicators)}]")
