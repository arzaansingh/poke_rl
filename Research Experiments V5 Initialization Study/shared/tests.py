"""
Comprehensive test suite for the V5 research experiment.
...
Tests cover:
  ...
  4. Smart init (heuristic seeding, bounded standardized priors)
  ...
  8. Save/load round-trip (pickle correctness for all models)
  9. Integration (end-to-end short battle for all models)
"""
...
def test_config():
    ...
    from shared.config import (
        ...
        ALPHA, GAMMA, LAMBDA, MODEL_NAMES, MODEL_LABELS, get_epsilon, resolve_epsilon,
        FIXED_EPSILON, MODEL_EPSILON_MODE,
        SMART_INIT_Q_MIN, SMART_INIT_Q_MAX,
    ...
    _test("Fixed epsilon matches terminal epsilon", abs(FIXED_EPSILON - EPS_END) < 1e-6)
    _test("resolve_epsilon fixed mode returns fixed epsilon", abs(resolve_epsilon(0, mode="fixed") - FIXED_EPSILON) < 1e-6)
    _test("Smart init min = -0.01", abs(SMART_INIT_Q_MIN + 0.01) < 1e-6)
    _test("Smart init max = 0.01", abs(SMART_INIT_Q_MAX - 0.01) < 1e-6)
    _test("8 model names", len(MODEL_NAMES) == 8)
    _test("8 model labels", len(MODEL_LABELS) == 8)
    _test("Epsilon mode mapped for all models", set(MODEL_EPSILON_MODE) == set(MODEL_NAMES))
...
def test_flat_smart_player():
    ...
    # After init: all Q-values should be bounded in [SMART_INIT_Q_MIN, SMART_INIT_Q_MAX].
    ...
    _test("after init: Q-values stay within configured bounds",
          all(SMART_INIT_Q_MIN <= q <= SMART_INIT_Q_MAX for q in q_after), str(q_after))
    _test("after init: best prior reaches configured max",
          abs(max(q_after) - SMART_INIT_Q_MAX) < 1e-6, str(q_after))
    _test("after init: worst prior reaches configured min",
          abs(min(q_after) - SMART_INIT_Q_MIN) < 1e-6, str(q_after))
...
def test_hier_smart_player():
    ...
    _test("master smart init: values bounded",
          all(SMART_INIT_Q_MIN <= q <= SMART_INIT_Q_MAX for q in q_vals), str(q_vals))
    _test("master smart init: best prior reaches configured max",
          abs(max(q_vals) - SMART_INIT_Q_MAX) < 1e-6, str(q_vals))
    _test("master smart init: worst prior reaches configured min",
          abs(min(q_vals) - SMART_INIT_Q_MIN) < 1e-6, str(q_vals))
    ...
    for mon in candidates:
        ...
        switch_keys.append((sub_state, sh))
        switch_vals.append(p.get_switch_value(sub_state, sh))
    _test("switch smart init: all candidates initialized", all(k in p.switch_table for k in switch_keys))
    _test("switch smart init: values bounded",
          all(SMART_INIT_Q_MIN <= v <= SMART_INIT_Q_MAX for v in switch_vals), str(switch_vals))
...
def test_indexed_team_builder():
    ...
    # Cross-model determinism: all models should see the same team stream.
    model_tbs = [IndexedTeambuilder(base_seed=42) for _ in range(len(__import__("shared.config", fromlist=["MODEL_NAMES"]).MODEL_NAMES))]
    for battle_idx in [0, 50, 999]:
        teams = []
        for tb in model_tbs:
            tb.battle_index = battle_idx
            teams.append(tb.yield_team())
        _test(f"all models get same team at index {battle_idx}",
              len(set(teams)) == 1)
...
def test_grid_search():
    ...
    # --- Port allocation ---
    sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))
    from run_grid import _get_port, _order_combos
    ...
    reversed_combos = _order_combos(combos, reverse_order=True)
    _test("reverse combo order starts at the end",
          [c["combo_id"] for c in reversed_combos[:3]] == [c["combo_id"] for c in combos[-1:-4:-1]])
...
def run_all_tests(skip_integration=False):
    ...
    print("\n" + "=" * 60)
    print("  RESEARCH EXPERIMENT V5 — INITIALIZATION STUDY TEST SUITE")
    print("=" * 60)
