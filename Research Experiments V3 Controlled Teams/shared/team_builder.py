...
class Gen4OUPoolTeambuilder(Teambuilder):
    """
    Teambuilder that generates random teams from a fixed pool of gen4ou Pokemon.
...
class IndexedTeambuilder(Teambuilder):
    """
    Deterministic team builder where battle N always produces the same team,
    regardless of previous battles, timeouts, or retries.

    Uses (base_seed, battle_index) as the RNG seed for each battle.
    This guarantees that all 4 models in the experiment face identical
    team matchups for every battle, making the only variable the model itself.

    Set `self.battle_index` before each battle from the training loop.
    """

    def __init__(self, pool=None, base_seed=0):
        self.pool = pool or POKEMON_POOL
        self.base_seed = base_seed
        self.battle_index = 0

    def yield_team(self):
        # Fresh RNG seeded by (base_seed, battle_index) — deterministic per battle
        rng = _random.Random(f"{self.base_seed}_{self.battle_index}")
        team_paste = _build_random_team(self.pool, rng)
        parsed = self.parse_showdown_team(team_paste)
        return self.join_team(parsed)
