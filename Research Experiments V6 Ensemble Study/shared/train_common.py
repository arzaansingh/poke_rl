...
from shared.config import (
    ...
    ALPHA, GAMMA, LAMBDA, resolve_epsilon
)


class GracefulExit(BaseException):
    """Signalled when the training loop should save and exit cleanly.
    Extends BaseException so `except Exception:` inside the loop doesn't
    swallow it. Raised synchronously at a safe point between battles
    (not from inside a signal handler, because asyncio doesn't handle
    exceptions raised from signal handlers reliably)."""
    pass


# Module-level flag set by signal handlers; checked between battles.
_shutdown_reason: list = []


def _install_signal_handlers():
    """Install SIGTERM + SIGINT handlers that set a flag.  The training loop
    checks the flag after each battle and raises GracefulExit at a safe
    point.  This is asyncio-safe — raising from the handler directly can
    propagate through selector.select() and escape the outer try/except."""
    def _handle(signum, frame):
        name = {signal.SIGTERM: "SIGTERM", signal.SIGINT: "SIGINT (Ctrl+C)"}.get(
            signum, f"signal {signum}"
        )
        # Only record first signal; second Ctrl+C should still work to
        # force-exit via default handling.
        if not _shutdown_reason:
            _shutdown_reason.append(name)
            # Print from the handler so the user gets immediate feedback.
            print(f"\n[shutdown] Received {name} — will save after current battle...",
                  flush=True)
    try:
        ...
        # subthreads we just skip.
        pass


def _shutdown_requested() -> bool:
    return bool(_shutdown_reason)


def _shutdown_name() -> str:
    return _shutdown_reason[0] if _shutdown_reason else "unknown"

# These can be overridden per-run via CLI args (--alpha, --gamma, --lam)
...
async def run_training(player_class, args, model_dir):
    ...
    Graceful shutdown: Ctrl+C (SIGINT) and SIGTERM both save the current
    Q-table to disk before exiting, so interrupted runs never lose more than
    the last completed battle.
    """
    _install_signal_handlers()
    run_uuid = uuid.uuid4().hex[:8]
    server_config = _get_server_config(getattr(args, 'port', 8000))

    # Create directories — use --output_dir if provided (grid search), else model_dir
    base_dir = getattr(args, 'output_dir', None) or model_dir
    ...
    # If --pool is provided, use that specific pokemon pool (V4 grid search).
    pool = getattr(args, 'pool', None)
    if pool and isinstance(pool, str):
        pool = pool.split(",")
    opp_tb = IndexedTeambuilder(pool=pool, base_seed=args.seed + 1000)
    learner_tb = IndexedTeambuilder(pool=pool, base_seed=args.seed + 2000)
    ...
    # Learner — use CLI overrides if provided, else config defaults
    hp_alpha = getattr(args, 'alpha', None) or ALPHA
    hp_gamma = getattr(args, 'gamma_val', None) or GAMMA
    hp_lam = getattr(args, 'lam', None) or LAMBDA
    ...
    learner = LearnerClass(
        ...
        server_configuration=server_config,
        max_concurrent_battles=1,
        alpha=hp_alpha, gamma=hp_gamma, lam=hp_lam, epsilon=args.epsilon,
        team=learner_tb,
    )
    ...
    status_file = os.path.join(base_dir, f"live_status_run_{args.run_id}.json")
    ...
    if args.historic_battles == 0:
        start_epsilon = resolve_epsilon(0, mode=args.epsilon_mode, fixed_epsilon=args.fixed_epsilon)
        print(
            f"--- {player_class.__name__} Run {args.run_id} "
            f"(Batch: {args.batch_size}, Eps: {start_epsilon:.3f}, Mode: {args.epsilon_mode}) ---"
        )

    def _graceful_shutdown(reason):
        """Save the Q-table and clean up on SIGINT / SIGTERM. Always runs
        before the process exits. Idempotent."""
        total = args.historic_battles + battles_collected
        print(
            f"\n[shutdown] {reason} at {total:,} battles "
            f"(session: {battles_collected:,}) — saving Q-table...",
            flush=True,
        )
        try:
            learner.save_table(model_file)
            print(f"[shutdown] saved to {model_file}", flush=True)
        except Exception as save_err:
            print(f"[shutdown] SAVE FAILED: {save_err}", flush=True)
            traceback.print_exc()
        try:
    ...
    try:
        while battles_collected < args.batch_size:
            # Check for shutdown signal at a safe point (between battles,
            # before we touch the network). This is asyncio-safe.
            if _shutdown_requested():
                raise GracefulExit(_shutdown_name())
            ...
            try:
                ...
                learner.epsilon = resolve_epsilon(
                    battle_idx,
        ...
        _graceful_shutdown("KeyboardInterrupt (Ctrl+C)")
        sys.exit(130)
    except GracefulExit as e:
        _graceful_shutdown(f"SIGTERM ({e})")
        sys.exit(143)
...
def parse_train_args():
    ...
    # HP overrides for grid search (V4)
    parser.add_argument("--alpha", type=float, default=None)
    parser.add_argument("--gamma_val", type=float, default=None)
    parser.add_argument("--lam", type=float, default=None)
    parser.add_argument("--output_dir", type=str, default=None)
    parser.add_argument("--pool", type=str, default=None,
                        help="Comma-separated pokemon pool for this run")
    parser.add_argument("--epsilon_mode", choices=["decay", "fixed"], default="decay")
    parser.add_argument("--fixed_epsilon", type=float, default=None)
