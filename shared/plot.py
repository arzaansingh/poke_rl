...
Usage:
  python shared/plot.py --combo hp_001                  # One combo, all 4 models
...
def plot_combo(combo_id, models=None, metric='RollingWin', save=True):
    """Plot all 4 models for one HP combo: mean + std band, individual runs faint."""
...
def plot_heatmap(metric='RollingWin', save=True):
    ...
    ax.set_title("V4 Grid Search \u2014 Best Rolling Win Rate by Combo", fontsize=13, fontweight='bold')
    ax.set_ylabel("")
    plt.tight_layout()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="V4 Grid Search Plots")
    ...
    if args.heatmap:
        ...
        # Single combo: overlay all 4 models
        if args.panel:
