...
def test_config():
    ...
    from shared.config import (
        ...
        BATTLES_PER_RUN, RUNS_PER_MODEL, RANDOM_SEEDS,
    ...
    _test(f"Pool has {POOL_SIZE} Pokemon", len(POKEMON_POOL) == POOL_SIZE, f"got {len(POKEMON_POOL)}")
    _test("Pool size >= 6 (need full teams)", POOL_SIZE >= 6, f"got {POOL_SIZE}")
    _test("Pool species are unique", len(set(POKEMON_POOL)) == POOL_SIZE)
...
def test_parallel_runs():
    ...
    _test("parallel port (1,0) = 9000+RUNS", _get_parallel_run_port(1, 0) == 9000 + _RUNS)
    _test("parallel port (3,4) = 9000+3*RUNS+4", _get_parallel_run_port(3, 4) == 9000 + 3 * _RUNS + 4)

    # All ports are unique
    expected_total = 4 * _RUNS
    all_ports = set()
    for m in range(4):
        for r in range(_RUNS):
            all_ports.add(_get_parallel_run_port(m, r))
    _test(f"all {expected_total} parallel ports are unique", len(all_ports) == expected_total)
...
def run_all_tests(skip_integration=False):
    ...
    print("\n" + "=" * 60)
    print("  RESEARCH EXPERIMENT V2 — COMPREHENSIVE TEST SUITE")
    print("=" * 60)
