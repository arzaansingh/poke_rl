"""
Model 3: Hierarchical + Zero Init
====================================
Master agent: 20-tuple state (battle_state), actions: {move hashes, -1=switch}
Sub-agent:    17-tuple state (opp context + sorted bench detail), actions: {switch species hashes}
Both tables zero-initialized.
"""
...
class HierZeroPlayer(Player):
    def __init__(self, battle_format="gen4ou", alpha=0.1, gamma=0.995, lam=0.6967, epsilon=0.1, **kwargs):
        ...
        # Master table: (battle_state_20, action_hash) → Q
        self.q_table = {}
        # Sub table: (sub_state_17, switch_species_hash) → Q
        self.switch_table = {}
    ...
    def choose_move(self, battle):
        ...
        # Build master actions: moves + switch option
        possible_actions = []
    ...
    def _sub_agent_switch(self, battle, parent_greedy):
        """Sub-agent picks which Pokemon to switch to."""
